...
export default function EditPost() {
      ...
      const { data: postData, error: postError } = await supabase
        ...
        .select(`
          *,
          ...
          blog_post_tags(tag_id, blog_tags(*))
        `)
        .eq('id', postId)
        .single()
  ...
  const updatePost = async (publishStatus: 'draft' | 'published') => {
    ...
    try {
      ...
      const postData = {
        ...
        published_at: publishStatus === 'published' && post?.status === 'draft' 
      ...
      alert(publishStatus === 'published' ? '記事を公開しました' : '記事を更新しました')
      router.push('/admin/dashboard')
      ...
      if (error && typeof error === 'object' && 'message' in error) {
        ...
        if (errorMessage.includes('violates row-level security')) {
          alert('更新に失敗しました（認証エラー）。ページをリフレッシュして再試行してください。')
        } else if (errorMessage.includes('duplicate key')) {
          alert('更新に失敗しました（重複エラー）。タイトルを変更して再試行してください。')
        } else if (errorMessage.includes('foreign key')) {
          alert('更新に失敗しました（関連データエラー）。カテゴリやタグを確認してください。')
        } else {
